...
import com.github.hurdad.storm.forex.bolt.StochTest.TupleType;
...
public class PercRTest {
	...
	public void shouldEmitPercRValues() {
		...
		// given
		PercRBolt bolt = new PercRBolt(14);
		...
		// then
		verify(collector).emit(new Values("mypair", 1268352000, "-29.56"));
		verify(collector).emit(new Values("mypair", 1268611200, "-32.39"));
		verify(collector).emit(new Values("mypair", 1268697600, "-10.80"));
		verify(collector).emit(new Values("mypair", 1268784000, "-34.19"));
		verify(collector).emit(new Values("mypair", 1268870400, "-18.25"));
		verify(collector).emit(new Values("mypair", 1268956800, "-35.48"));
		verify(collector).emit(new Values("mypair", 1269216000, "-25.47"));
		verify(collector).emit(new Values("mypair", 1269302400, "-1.42"));
		verify(collector).emit(new Values("mypair", 1269388800, "-29.90"));
		verify(collector).emit(new Values("mypair", 1269475200, "-26.94"));
		verify(collector).emit(new Values("mypair", 1269561600, "-26.58"));
		verify(collector).emit(new Values("mypair", 1269820800, "-38.77"));
		verify(collector).emit(new Values("mypair", 1269907200, "-39.04"));
		verify(collector).emit(new Values("mypair", 1269993600, "-59.61"));
		verify(collector).emit(new Values("mypair", 1270080000, "-59.61"));
		verify(collector).emit(new Values("mypair", 1270425600, "-33.17"));
		verify(collector).emit(new Values("mypair", 1270512000, "-43.27"));
